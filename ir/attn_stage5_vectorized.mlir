...
#map9 = affine_map<(d0, d1, d2) -> (d0, 0, d2, d1)>
...
module {
  func.func public @main(%arg0: memref<1x128x128xf32, strided<[?, ?, ?], offset: ?>>, %arg1: memref<1x128x128xf32, strided<[?, ?, ?], offset: ?>>, %arg2: memref<1x128x128xf32, strided<[?, ?, ?], offset: ?>>, %arg3: memref<1x128x128xf32, strided<[?, ?, ?], offset: ?>>) -> memref<1x128x128xf32> {
          ...
          scf.for %arg7 = %c0 to %c64 step %c4 {
            scf.for %arg8 = %c0 to %c64 step %c4 {
              scf.for %arg9 = %c0 to %c64 step %c4 {
                ...
                %0 = vector.transfer_read %subview_29[%c0, %c0], %cst {in_bounds = [true, true, true], permutation_map = #map} : memref<4x4xf32, strided<[128, 1], offset: ?>>, vector<4x4x4xf32>
                %1 = vector.transfer_read %subview_30[%c0, %c0], %cst {in_bounds = [true, true, true], permutation_map = #map1} : memref<4x4xf32, strided<[128, 1], offset: ?>>, vector<4x4x4xf32>
                %2 = vector.transfer_read %subview_31[%c0, %c0], %cst {in_bounds = [true, true]} : memref<4x4xf32, strided<[128, 1], offset: ?>>, vector<4x4xf32>
                %3 = vector.contract {indexing_maps = [#map2, #map2, #map3], iterator_types = ["parallel", "parallel", "reduction"], kind = #vector.kind<add>} %0, %1, %2 : vector<4x4x4xf32>, vector<4x4x4xf32> into vector<4x4xf32>
                vector.transfer_write %3, %subview_31[%c0, %c0] {in_bounds = [true, true]} : vector<4x4xf32>, memref<4x4xf32, strided<[128, 1], offset: ?>>
              } {done_tiling}
            } {done_tiling}
          } {done_tiling}
        } {done_tiling}
    ...
    linalg.generic {indexing_maps = [#map4, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expand_shape : memref<1x128x4x32xf32>) outs(%alloc_14 : memref<1x4x128x32xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    %alloc_15 = memref.alloc() {alignment = 64 : i64} : memref<1x4x32x128xf32>
    linalg.generic {indexing_maps = [#map6, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expand_shape_11 : memref<1x128x4x32xf32>) outs(%alloc_15 : memref<1x4x32x128xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
    linalg.generic {indexing_maps = [#map4, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expand_shape_12 : memref<1x128x4x32xf32>) outs(%alloc_13 : memref<1x4x128x32xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
        ...
        scf.for %arg6 = %c0 to %c64 step %c4 {
          scf.for %arg7 = %c0 to %c64 step %c4 {
            scf.for %arg8 = %c0 to %c32 step %c4 {
              ...
              %1 = vector.transfer_read %subview_29[%c0, %c0, %c0], %cst {in_bounds = [true, true, true, true], permutation_map = #map8} : memref<4x4x?xf32, strided<[4096, 32, 1], offset: ?>>, vector<4x4x4x4xf32>
              %2 = vector.transfer_read %subview_30[%c0, %c0, %c0], %cst {in_bounds = [true, true, true, true], permutation_map = #map9} : memref<4x?x4xf32, strided<[4096, 128, 1], offset: ?>>, vector<4x4x4x4xf32>
              %3 = vector.transfer_read %subview_31[%c0, %c0, %c0], %cst {in_bounds = [true, true, true]} : memref<4x4x4xf32, strided<[16384, 128, 1], offset: ?>>, vector<4x4x4xf32>
              %4 = vector.contract {indexing_maps = [#map5, #map5, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction"], kind = #vector.kind<add>} %1, %2, %3 : vector<4x4x4x4xf32>, vector<4x4x4x4xf32> into vector<4x4x4xf32>
              vector.transfer_write %4, %subview_31[%c0, %c0, %c0] {in_bounds = [true, true, true]} : vector<4x4x4xf32>, memref<4x4x4xf32, strided<[16384, 128, 1], offset: ?>>
            } {done_tiling}
          } {done_tiling}
        } {done_tiling}
      } {done_tiling}
    } {done_tiling}
    linalg.generic {indexing_maps = [#map2, #map2], iterator_types = ["parallel", "parallel", "parallel"]} ins(%alloc_20 : memref<4x128x128xf32>) outs(%alloc_19 : memref<4x128x128xf32>) {
    ^bb0(%in: f32, %out: f32):
    ...
    linalg.generic {indexing_maps = [#map2, #map3], iterator_types = ["parallel", "parallel", "reduction"]} ins(%alloc_19 : memref<4x128x128xf32>) outs(%alloc_22 : memref<4x128xf32>) {
    ^bb0(%in: f32, %out: f32):
      %0 = arith.cmpf ogt, %in, %out : f32
      %1 = arith.select %0, %in, %out : f32
      linalg.yield %1 : f32
    }
    linalg.generic {indexing_maps = [#map2, #map3, #map2], iterator_types = ["parallel", "parallel", "parallel"]} ins(%alloc_20, %alloc_22 : memref<4x128x128xf32>, memref<4x128xf32>) outs(%alloc_19 : memref<4x128x128xf32>) {
    ...
    linalg.generic {indexing_maps = [#map2, #map3], iterator_types = ["parallel", "parallel", "reduction"]} ins(%alloc_19 : memref<4x128x128xf32>) outs(%alloc_21 : memref<4x128xf32>) {
    ^bb0(%in: f32, %out: f32):
      %0 = arith.addf %in, %out : f32
      linalg.yield %0 : f32
    }
    linalg.generic {indexing_maps = [#map2, #map3, #map3, #map2], iterator_types = ["parallel", "parallel", "parallel"]} ins(%alloc_20, %alloc_22, %alloc_21 : memref<4x128x128xf32>, memref<4x128xf32>, memref<4x128xf32>) outs(%alloc_19 : memref<4x128x128xf32>) {
        ...
        scf.for %arg6 = %c0 to %c64 step %c4 {
          scf.for %arg7 = %c0 to %c32 step %c4 {
            scf.for %arg8 = %c0 to %c64 step %c4 {
              ...
              %1 = vector.transfer_read %subview_29[%c0, %c0, %c0], %cst {in_bounds = [true, true, true, true], permutation_map = #map8} : memref<4x4x4xf32, strided<[16384, 128, 1], offset: ?>>, vector<4x4x4x4xf32>
              %2 = vector.transfer_read %subview_30[%c0, %c0, %c0], %cst {in_bounds = [true, true, true, true], permutation_map = #map9} : memref<4x4x?xf32, strided<[4096, 32, 1], offset: ?>>, vector<4x4x4x4xf32>
              %3 = vector.transfer_read %subview_31[%c0, %c0, %c0], %cst {in_bounds = [true, true, true]} : memref<4x4x?xf32, strided<[4096, 32, 1], offset: ?>>, vector<4x4x4xf32>
              %4 = vector.contract {indexing_maps = [#map5, #map5, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction"], kind = #vector.kind<add>} %1, %2, %3 : vector<4x4x4x4xf32>, vector<4x4x4x4xf32> into vector<4x4x4xf32>
              vector.transfer_write %4, %subview_31[%c0, %c0, %c0] {in_bounds = [true, true, true]} : vector<4x4x4xf32>, memref<4x4x?xf32, strided<[4096, 32, 1], offset: ?>>
            } {done_tiling}
          } {done_tiling}
        } {done_tiling}
      } {done_tiling}
    ...
    linalg.generic {indexing_maps = [#map4, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expand_shape_24 : memref<1x4x128x32xf32>) outs(%alloc_25 : memref<1x128x4x32xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    }
